		...
		(
			...
			FROM(
						  SELECT NS.Счет
								 , SUM(NS.Сумма) AS Summa
							FROM stack.НСальдо AS NS
							WHERE  NS.[Месяц расчета]=DATEADD(mm, -1, @date) AND NS.[Номер услуги]  BETWEEN 300 AND 399
							GROUP BY NS.Счет, NS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=DATEADD(mm, -1, @date) AND VS.[Номер услуги]  BETWEEN 3700 AND 3799
							GROUP BY VS.Счет, VS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=DATEADD(mm, -1, @date) AND VS.[Номер услуги]  BETWEEN 3800 AND 3899
							GROUP BY VS.Счет, VS.[Номер услуги]							
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=DATEADD(mm, -1, @date) AND VS.[Номер услуги]  BETWEEN 3900 AND 3999
							GROUP BY VS.Счет, VS.[Номер услуги]							
							) AS S
					GROUP BY S.Счет
		),
		...
		(
			...
			FROM(
						  SELECT NS.Счет
								 , SUM(NS.Сумма) AS Summa
							FROM stack.НСальдо AS NS
							WHERE  NS.[Месяц расчета]= @date AND NS.[Номер услуги]  BETWEEN 100 AND 199
							GROUP BY NS.Счет, NS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=@date AND VS.[Номер услуги]  BETWEEN 400 AND 499
							GROUP BY VS.Счет, VS.[Номер услуги]) AS S
			GROUP BY S.Счет
		),
		...
		(
			...
			FROM(
						  SELECT NS.Счет
								 , SUM(NS.Сумма) AS Summa
							FROM stack.НСальдо AS NS
							WHERE  NS.[Месяц расчета]= @date AND NS.[Номер услуги]  BETWEEN 300 AND 399
							GROUP BY NS.Счет, NS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=@date AND VS.[Номер услуги]  BETWEEN 3700 AND 3799
							GROUP BY VS.Счет, VS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=@date AND VS.[Номер услуги]  BETWEEN 3800 AND 3899
							GROUP BY VS.Счет, VS.[Номер услуги]
						UNION
							SELECT VS.Счет
								 , SUM(VS.Сумма) AS Summa
							FROM stack.НСальдо AS VS
							WHERE   VS.[Месяц расчета]=@date AND VS.[Номер услуги]  BETWEEN 3900 AND 3999
							GROUP BY VS.Счет, VS.[Номер услуги]) AS S
			GROUP BY S.Счет
		),
--платеж
...
--телефон и e-mail
--нужна проверка на тип и в соответствие с ним разнесение по столбцам+склейка в одну строку, если несколько
PHONE
	AS
		(
			SELECT 
...
EMAILS
 ...
 SELECT	
		 ...
		 ,SC.Разрядность
		 ,SC.[Коэф.трансф]
		 ,POK.[Дата последних показаний день]
		 ,POK.[Последние показания день]
		 ,POK.[Тип последнего показания день]
		 ,POK.[Дата предпоследних показаний день]
		 ,POK.[предыдущие показания день]
		 ,POK.[Тип предпоследнего показания день]
		 ,POK.[Дата последних показаний ночь]
		 ,POK.[Последние показания ночь]
		 ,POK.[Тип последнего показания ночь]
		 ,POK.[Дата предыдущих показаний ночь]
		 ,POK.[предыдущие показания ночь]
		 ,POK.[Тип предпоследнего показания ночь]
		 ,ISNULL(POK.[Дата последних показаний ППик], ' ') AS [Дата последних показаний ППик]
		 ,ISNULL(POK.[Последние показания ППик],' ') AS [Последние показания ППик]
		 ,ISNULL(POK.[Тип последнего показания ППик],' ') AS [Тип последнего показания ППик]
		 ,ISNULL(POK.[Дата предыдущих показаний ППик],' ') AS [Дата предыдущих показаний ППик]
		 ,ISNULL(POK.[предыдущие показания ППик],' ') AS [предыдущие показания ППик]
		 ,ISNULL(POK.[Тип предпоследнего показания ППик],' ') AS [Тип предпоследнего показания ППик]
