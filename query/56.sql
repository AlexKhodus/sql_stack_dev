...
DECLARE @date datetime='20201001',
		@account int=161408;
	...
	AS
		(
				  SELECT LI.Потомок			
					...
					JOIN stack.[Управляющие компании] AS UK_DOM
...
ODPU
	...
	SELECT 
		...
		JOIN stack.[Состояние счетчика] AS SS
			ON SS.[Счет-Счетчика состояние]=LI.Родитель 
			AND SS.Состояние!=3 
...
BU
	...
	(
		SELECT SUM(NT.Сумма) [Стоимость по Акту БУ]
			   ...
			   ,NT.Счет AS Потомок
		FROM stack.НТариф AS NT
		WHERE NT.[Номер услуги] in (
		SELECT  TU.[Номер услуги]
...
PARAMETRS AS (
	...
	AS
		(
			...
			(
				SELECT LI.Потомок,
					ISNULL(TU.ROW_ID,TUD.ROW_ID) AS ROW_ID,
					ISNULL(TU.[Номер услуги],TUD.[Номер услуги]) AS [Номер услуги],
					ISNULL(TU.Наименование,TUD.Наименование) AS [Тип домохозяйства]
				FROM stack.[Лицевые иерархия] AS LI
				--для ЛС
				LEFT JOIN stack.[Список услуг] AS SU ON SU.[Счет-Услуги]=LI.Потомок 
				LEFT JOIN stack.[Типы услуг] AS TU ON TU.ROW_ID=SU.[Вид-Услуги]
			--для дома
				LEFT JOIN stack.[Список услуг] AS SUD ON SUD.[Счет-Услуги]=LI.Родитель 
				LEFT JOIN stack.[Типы услуг] AS TUD ON TUD.ROW_ID=SUD.[Вид-Услуги]
				WHERE LI.РодительТип=3 AND LI.ПотомокТип=5 
			) AS Z
			OUTER APPLY 
			(
